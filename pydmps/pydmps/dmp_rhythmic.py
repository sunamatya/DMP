...
class DMPs_rhythmic(DMPs):
    ...
    def gen_centers(self):
        ...
        c = np.linspace(0, 2*np.pi, self.n_bfs+1) #1000 (converting 0 t0 2pi in 1000 steps)
        c = c[0:-1]# selecting whole array
        self.c = c
    ...
    def gen_psi(self, x):
        ...
        if isinstance(x, np.ndarray):
            x = x[:, None]
        return np.exp(self.h * (np.cos(x - self.c) - 1)) #equation 2.7
    ...
    def gen_weights(self, f_target):
        ...
        psi_track = self.gen_psi(x_track) # equation 2.7

        # efficiently calculate BF weights using weighted linear regression
        for d in range(self.n_dmps): # equation 2.6 amplutide signal r is 1 here
